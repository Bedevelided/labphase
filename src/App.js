...
import {searchMovie, bookMarkUnBookBookmark } from './JS/Actions/actions';
...
const mapDispatchToProps = (dispatch) => {
  return {
    bookMarkUnBookBookmark : (title) => dispatch(bookMarkUnBookBookmark(title)),
...
function App({movies , searchMovie , bookMarkUnBookBookmark}) {
  // console.log(movies)
  useEffect(() => {
  ...
  return (
    <div className="container">
        <aside>
            ...
            <div className='avatar' style={{width : "50px" , height: "50px" , border : "1px solid white" , borderRadius : "50%"}}>
                <img style={{width : "100%"}} src={avatar} alt="Avatar"/>
            </div>
        </aside>
        <main>
            <Routes>
              <Route path='/' element={<Home movies = {movies} bookMarkUnBookBookmark ={bookMarkUnBookBookmark} searchMovie={searchMovie}/>}></Route>
              <Route path='/movies' element={<Movies  movies = {movies} bookMarkUnBookBookmark={bookMarkUnBookBookmark} searchMovie={searchMovie}/>}></Route>
              <Route path='/tvseries' element={<Tvseries  movies = {movies} bookMarkUnBookBookmark={bookMarkUnBookBookmark} searchMovie={searchMovie}/>}></Route>
              <Route path='/bookmarked' element={<Bookmarked  movies = {movies} bookMarkUnBookBookmark={bookMarkUnBookBookmark} searchMovie={searchMovie}/>}></Route>
            </Routes>
        </main>
    </div>
  );
