...
const Movieitem = ({movies , category , bookMarkUnBookBookmark}) => {
  // console.log(movies[0].thumbnail.regular.medium)
  let movieTypes = []
  if(category) {
     movieTypes = movies.filter((item) => {
  ...
  return (
    <div className='all__movies'>
        ...
        {movieTypes.map((movie , index) => {
          const item = movie.thumbnail
          return <div className="movie" key={index}>
              <div className="movie__img">
                <div className='bg__movie'>
                    <img  style={{borderRadius : "10px" , overflow: "hidden"}} src={item.regular.medium} alt={movie.title}/>
                </div>
                <button className='btn btn-bookmark' onClick={() => {bookMarkUnBookBookmark(movie.title)}}>
                  {movie.isBookmarked ?
              ...
              <div className="movie__details" style={{fontSize : "0.85rem", fontWeight : 300}}>
                <ul className='movie__info' style={{marginTop : "1rem"}}>
                   <li style={{listStyle : 'none'}}>{movie.year}</li>
                   <li> 
                    {movie.category === "Movie" ? <span style={{paddingRight : "0.35rem"}}>
                    ...
                    </span> : <span style={{paddingRight : "0.35rem"}}>
                ...
                <h3 style={{fontSize : "1.15rem" , marginTop : "0.65rem"}}>{movie.title}</h3>
              </div>
          </div>
        })}
    </div>
